...
const SermonDisplay: React.FC<SermonDisplayProps> = ({ sermon }) => {
  ...
  const handleCopyToClipboard = async () => {
    const sermonText = `
PREGAÇÃO: ${sermon.mainMessage.title}
Baseada em: ${sermon.keyVerse.reference}
...
${sermon.illustrations.map((ill, index) => `${index + 1}. ${ill}`).join('\n')}
    ...
    try {
      ...
      toast({
        title: "Copiado!",
        description: "Pregação copiada para a área de transferência.",
      });
    } catch (error) {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-6 animate-fade-in">
      {/* Header com informações da pregação */}
