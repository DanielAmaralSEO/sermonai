...
const SermonDisplay: React.FC<SermonDisplayProps> = ({ sermon }) => {
  ...
  const handleCopyToClipboard = async () => {
    const sermonText = `
SERMÓN: ${sermon.mainMessage.title}
Basado en: ${sermon.keyVerse.reference}
${sermon.request.theme ? `Tema: ${sermon.request.theme}` : ''}
...
${sermon.illustrations.map((ill, index) => `${index + 1}. ${ill}`).join('\n')}
    ...
    try {
      ...
      toast({
        title: "¡Copiado!",
        description: "Sermón copiado al portapapeles.",
      });
    } catch (error) {
  ...
  const handleDownloadPDF = () => {
    toast({
      ...
      description: "La descarga en PDF estará disponible pronto.",
    });
  };

  return (
    <div className="w-full max-w-4xl mx-auto space-y-6 animate-fade-in">
      {/* Header con información del sermón */}
